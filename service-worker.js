...
self.addEventListener('fetch', e=>{
  ...
  // Stale‑While‑Revalidate برای فایل‌های استاتیک همان دامنه
  ...
  // Network‑First برای درخواست‌های خارجی
  e.respondWith((async ()=>{
    try{ return await fetch(request); }
    catch{
      const cache = await caches.open(CACHE);
      return await cache.match(request) || Promise.reject();
    }
  })());
});
